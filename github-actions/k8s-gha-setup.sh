...
sudo sed -i -e 's/pause:3.8/pause:3.9/' /etc/containerd/config.toml
...
sudo kubeadm init --cri-socket=unix:///var/run/containerd/containerd.sock --kubernetes-version="${K8S_VERSION}" --apiserver-cert-extra-sans=$(curl http://checkip.amazonaws.com)
mkdir -p "${HOME}/.kube"
