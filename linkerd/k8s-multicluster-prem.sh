...
sudo kubeadm init --cri-socket=unix:///var/run/containerd/containerd.sock --kubernetes-version="${K8S_VERSION}" \
  --pod-network-cidr "10.120.0.0/16" --service-cidr "10.220.0.0/16" --service-dns-domain "prem.local"
mkdir -p "${HOME}/.kube"
...
# The --set ipam.mode=kubernetes switch tells cilium to use the Pod address ranges assigned by 
# Kubernetes (by default Cilium will ignore the K8s settings and use its own IPAM).
