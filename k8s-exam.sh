...
sudo apt-get update
sudo apt-get install -y kubeadm=1.22.0-00 kubectl=1.22.0-00 kubelet=1.22.0-00
sudo swapoff -a
if [ -z ${K8S_VERSION+x} ]; then K8S_VERSION="--kubernetes-version=stable-1" ; else K8S_VERSION="--kubernetes-version=$K8S_VERSION"; fi
sudo kubeadm init --cri-socket=unix:///run/cri-dockerd.sock  $K8S_VERSION --kubernetes-version=1.22.0
mkdir -p $HOME/.kube
